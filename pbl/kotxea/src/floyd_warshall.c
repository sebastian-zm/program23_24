...
GRAFOA* irakurri_eta_buruan_sartu(FILE * fitx, int*kopurua) {
	...
	ptr_hasiera = irakurri_fitxategitik_erpinak_diskreta(fitx,kopurua);
	return ptr_hasiera;
}



GRAFOA* irakurri_fitxategitik_erpinak_diskreta(FILE* fitx, int*kopurua) {
	...
	*kopurua = 0;
		...
		do {
			...
			ptr_berria = sortu_nodo_bat_diskreta(&zenbat_irakurri,fitx);
...
GRAFOA* sortu_nodo_bat_diskreta(int* zenbat_irakurri,FILE*fitx) {
	...
	*zenbat_irakurri = sscanf(lerroa, "%d %s %d %d", &ptr_elementua->elementua.id,ptr_elementua->elementua.izena, &ptr_elementua->elementua.x, &ptr_elementua->elementua.y);
...
void ikusi_ibilbidea(IBILBIDEA* ibilbidea) {
	...
	do {
		...
		if (hurrengoa->ptr_hurrengoa != NULL) {
			printf("-> biratu ");
			angelua = calcular_angulo(hurrengoa->elementua, ibilbidea->elementua, hurrengoa->ptr_hurrengoa->elementua);
			printf("%f", angelua);
			printf("->");
		}
...
void distantzia_hasieratu(GRAFOA* ptr_hasiera, float**distantziak) {
	...
	while (ptr_hasiera != NULL) { // ptr_hasiera.elememtnua.id --> kolumna erreferentia
		...
		while (ptr_ertzak_hasiera != NULL) {
			distantziak[ptr_hasiera->elementua.id][ptr_ertzak_hasiera->elementua.nora->id] = ptr_ertzak_hasiera->elementua.pisua;
...
float calcular_angulo(GRAFOA_ERPINA punto1, GRAFOA_ERPINA punto2, GRAFOA_ERPINA punto3) {
	...
	float coseno_theta = (magnitud_cuadrada_vector1 + magnitud_cuadrada_vector2 - magnitud_cuadrada_vector3) / (2 * magnitud_vector1 * magnitud_vector2);
