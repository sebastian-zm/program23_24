...
bool LOGIN_find_user(char* username, char* password, char* erabiltzaile_path) {
    ...
    while (!resolved && fgets(password_line, sizeof(password_line), passwords_file)) {
        sscanf(password_line, LOGIN_STR_FORMAT(LOGIN_MAX_USER_LENGTH) " " LOGIN_STR_FORMAT(LOGIN_MAX_PASS_LENGTH), current_username, current_password);
