...
void LIST_HELPER_cat(LIST_HELPER_NODE **list1, LIST_HELPER_NODE *list2)
{
	LIST_HELPER_NODE *node = *list1;
...
int LIST_HELPER_insertBefore(LIST_HELPER_NODE **list, void *data, int test(void *, void *), void *testData)
{
	int ret;
	LIST_HELPER_NODE *d = NULL;
	LIST_HELPER_NODE *prev = NULL;
	LIST_HELPER_NODE *current = *list;

	ret = LIST_HELPER_create(&d, data, NULL);

	if (ret) {
		while (current && !test(current->data, testData)) {
			prev = current;
			current = current->next;
		}
